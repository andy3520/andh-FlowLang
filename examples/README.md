# FlowLang Examples

This directory contains example programs written in FlowLang to demonstrate the language's capabilities.

## Examples

1. **[hello_world.flow](hello_world.flow)** - Classic "Hello, World!" program
2. **[fibonacci.flow](fibonacci.flow)** - Calculate Fibonacci numbers
3. **[sort.flow](sort.flow)** - Simple sorting algorithm
4. **[data_processing.flow](data_processing.flow)** - Data filtering and transformation

## Running Examples

These examples are currently reference implementations showing FlowLang syntax. 

As interpreters and tools are developed, this README will be updated with execution instructions.
